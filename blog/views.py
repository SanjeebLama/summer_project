    ...
    if request.method == 'POST':
        try:
            ...
            for user in User.objects.all():
                for camp in user.profile.camps.all():
                    if camp.name == search_input:
                        req_user_ids.append(user.id)

            req_users = User.objects.filter(id__in=req_user_ids)

            # first condition : post-0 , users-0 , camps-0
            if result_posts.count() == 0 and users.count() == 0 and req_users.count() == 0:
            ...
            elif users.count() == 0 and result_posts.count() != 0 and req_users.count() == 0:
                context = {'result_posts': result_posts, 'search_input': search_input, 'posts': posts}
                return render(request, 'blog/search.html', context)

            # 3rd condition : post-no , users-yes
            elif result_posts.count() == 0 and users.count() > 0 and req_users.count() > 0:
                context = {'users': users, 'posts': posts, 'search_input': search_input, 'req_users': req_users}
                return render(request, 'blog/search.html', context)
